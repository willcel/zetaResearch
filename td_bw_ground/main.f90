...
101 Continue
    ...
    do k=1,ns
        ...
        call cal_jacobi(rho_iter, hh_iter, jacobi1, nolayer, ntc)
    ...
    If(rerror>epsi .and. counter<2) Goto 101
    call MPI_Finalize(ierr)

10  Format(28E14.6)

20  Format(9E14.6)   ! 每个测点的参数个数 3*nolayers-1
...
360 ks = ks - 1
    ...
    subroutine cal_jacobi(m1, m2, jac, nlayer, ntc0)
    integer:: i,j,no,ntc0
    ...
    do j=1,no
        jac1 = 0.d0
        call cal_diff(m1, m2, zeta, delta, j, jac1, nlayer,ntc0)
        do i=1,ntc0
            jac(i,j)=jac1(i)
        end do
    end do
    ...
    subroutine cal_diff(rhoo, hhh, zeta1, del, jth, temp, nlayer,ntc1)

    integer:: i,k,jth,ntc1
    Real *8 zeta1(20),del, rhoo(nlayer), hhh(nlayer), temp(ntc1)
