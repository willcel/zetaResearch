    ...
    real *8,allocatable::hz2_iter(:),hz2obs(:)      ! dBz/dt VCX
    ...
    !   size2 = tot_ntc                ! ��Լ��
    size2 = tot_ntc+size1        ! ����Լ��
    !  size2 = tot_ntc+size1+size0  ! ���������Լ��
    ...
    allocate(hz2_iter(ntc),hz2obs(ntc))
    ...
    allocate(m_true(tot_para,1))
    allocate(para_mat(ns,nolayer),label(ns,nolayer))
...
101 Continue
    ...
    ! Write(18,*)'calculate Rp and Rd ...'


    do i=1+(2-1)*npara,(4-1)*npara
        Rp(i,i)=1
        Rp(i,i+npara)=-1
    end do
    ...
    !  deltam = matmul(matmul(invBtemp,transpose(Atemp)),b0)

    !  call cg_x(Atemp,deltam(:,1),b0(:,1),size2,tot_para)
...
20  Format(9E14.6)   ! ÿ�����Ĳ������� 3*nolayers-1
...
360 ks = ks - 1
    ...
    subroutine cal_jacobi(rank, nproc_current, Proc1, Proc2, m1, m2, jac, nlayer, ntc0)
    use mpi
    integer:: i,j,no,ntc0
    ...
    do j=1,size(npara_array)
        ...
        call cal_diff(m1,m2,zeta,workPProc2,delta,para_rank,jac_perRank,nlayer,ntc0)
    ...
    subroutine cal_diff(rhoo, hhh, zeta1, workPProc2, del, jth, temp, nlayer,ntc1)

    integer:: workPProc2
    integer:: i,k,jth,ntc1
    Real *8 zeta1(workPProc2),del, rhoo(nlayer), hhh(nlayer), temp(ntc1)
    Real *8 rhonew1(nlayer), hhnew1(nlayer), rhonew2(nlayer), hhnew2(nlayer)

    Real *8,allocatable:: hzz1(:), hzz2(:), hzz11(:), hzz22(:),time0(:)


    allocate(hzz1(ntc1), hzz2(ntc1))
    allocate(hzz11(ntc1), hzz22(ntc1),time0(ntc1))
    ...
    do i=1,size(zeta1)  ! different zeta
        ...
        call forwardprocess(rhonew1, hhnew1, hzz1, hzz2, nlayer,ntc1,time0,1)
        call forwardprocess(rhonew2, hhnew2, hzz11, hzz22, nlayer,ntc1,time0,1)
    ...
    Subroutine forwardprocess(rho, hh, hz1_pls, hz2_pls, nlayer,nt,t1,npls)
    integer::nlayer, nt,npls
    Real *8 t_st,t_ed,delta_t
    Real *8 t(npls*nt), hz1(npls*nt),tlog(npls*nt), hz2(npls*nt)

    Real *8 hz1_pls(nt),hz2_pls(nt),t1(nt)

    Real *8 x(1000), w(1000), s2, ss2
    Real *8 r, rplus, tt1, tt2, tt3, ti0, pi, tm
    Real *8 rho(nlayer), hh(nlayer), frq(67)

    Real *8 hx1, hy1, hx2, hy2, xr, yr, rt, ht, hr, zplus, zminus
    ...
    delta_t = (dlog10(t_ed)-dlog10(t_st))/nt
